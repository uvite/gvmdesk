...
// Highest
func (ta Ta) Highest(n int) float64 {
	rt := ta.vu.Runtime()
	h := rt.Get("high").Export().(*floats.Slice)

	return types.Highest(h, n)
}

// Highest
func (ta Ta) Lowest(n int) float64 {
	rt := ta.vu.Runtime()
	l := rt.Get("low").Export().(*floats.Slice)

	return types.Lowest(l, n)
}
...
func (ta Ta) Atr(n int64) floats.Slice {
	rt := ta.vu.Runtime()
	h := rt.Get("high").Export().(*floats.Slice)
	l := rt.Get("low").Export().(*floats.Slice)
	c := rt.Get("close").Export().(*floats.Slice)
	out := make(floats.Slice, c.Length())
...
func (ta Ta) Atr1(h, l, c floats.Slice, n int64) floats.Slice {
