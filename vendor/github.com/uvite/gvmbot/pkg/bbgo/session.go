...
import (
	...
	"github.com/uvite/gvmbot/pkg/cache"
	"github.com/uvite/gvmbot/pkg/util/templateutil"

	exchange2 "github.com/uvite/gvmbot/pkg/exchange"
	"github.com/uvite/gvmbot/pkg/fixedpoint"
	"github.com/uvite/gvmbot/pkg/service"
...
func (session *ExchangeSession) initSymbol(ctx context.Context, environ *Environment, symbol string) error {
	...
	if session.Exchange.Name() == "okex" {
		for interval := range klineSubscriptions {
			...
			for i = 0; i < KLinePreloadLimit; i += 200 {
				...
				kLines, err := session.Exchange.QueryKLines(ctx, symbol, interval, types.KLineQueryOptions{
					EndTime: &e,
					Limit:   200, // indicators need at least 100
				})
				if err != nil {
					return err
				}
	...
	if session.Exchange.Name() == "binance" {
		for interval := range klineSubscriptions {
			// avoid querying the last unclosed kline
			endTime := environ.startTime
			var i int64
