...
func convertPosition(risks []*futures.PositionRisk) ([]*types.Positions, error) {

	genvPostions := []*types.Positions{}

	for i, rsik := range risks {
		postion := &types.Positions{
			Id:               i,
			Symbol:           rsik.Symbol,
			PositionAmt:      rsik.PositionAmt,
			PositionSide:     rsik.PositionSide,
			EntryPrice:       rsik.EntryPrice,
			MarkPrice:        rsik.MarkPrice,
			Leverage:         rsik.Leverage,
			UnRealizedProfit: rsik.UnRealizedProfit,
			LiquidationPrice: rsik.LiquidationPrice,
		}
		genvPostions = append(genvPostions, postion)
	}
	return genvPostions, nil
